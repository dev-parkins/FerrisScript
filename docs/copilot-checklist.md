...
- [ ] **1.2** Create workspace structure with all crates
...
- [ ] **1.3** Fix Godot 4.x dependency (gdext instead of gdnative)
...
- [ ] **2.1** Implement Token enum and basic structure
  - Commit: `feat(compiler): define Token enum with all token types`
  - Complete Token enum with all variants
  - Add `#[derive(Debug, Clone, PartialEq)]`
  - Add helper methods for token display

- [ ] **2.2** Implement lexer core with whitespace and identifiers
  - Commit: `feat(compiler): implement lexer tokenization for keywords and identifiers`
  - Character iterator with peek
  - Keyword recognition (fn, let, mut, if, else, while)
  - Identifier parsing
  - Whitespace skipping
  - Basic error reporting with line/column tracking

- [ ] **2.3** Add number and string literal parsing
  - Commit: `feat(compiler): add number and string literal tokenization`
  - Integer and float parsing
  - String literal parsing with escape sequences
  - Boolean literals (true/false)

- [ ] **2.4** Add operators and delimiters
  - Commit: `feat(compiler): add operator and delimiter tokenization`
  - Single-char operators (+, -, *, /, etc.)
  - Multi-char operators (==, !=, <=, >=, &&, ||)
  - Delimiters (parentheses, braces, semicolons, commas)

- [ ] **2.5** Add lexer unit tests
  - Commit: `test(compiler): add comprehensive lexer unit tests`
