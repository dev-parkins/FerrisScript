...
impl TestHarness {
    ...
    pub fn run_script(&self, script_path: &Path) -> anyhow::Result<TestResult> {
        ...
        // Parse results
        let results = self.parser.parse(&script_name, &output.stdout, &output.stderr);
        
        // Determine pass/fail
        let passed = results.failed == 0 && 
                    self.parser.script_loaded_successfully(&output.stdout) &&
                    !self.parser.has_compilation_errors(&output.stdout, &output.stderr);
    ...
    fn generate_test_scene(
        ...
        let builder = if let Some(mut parsed) = crate::scene_builder::parse_scene_requirements(script_content) {
        ...
        Ok(PathBuf::from(format!("res://tests/generated/{}", scene_filename)))
    }
